@php use App\Models\Likes; @endphp
<script>
    function setLike(e) {
                    ...
                    <x-jstable>
                        @foreach($links as $link)
                            <tr>
                                ...
                                <td>
                                    @if(Likes::where('user_id', Auth::id())->where('links_id', $link->id)->exists())
                                        <button disabled @click="setLike" id="{{ $link->id }}"
                                    ...
                                    @else
                                        <button @click="setLike" id="{{ $link->id }}"
                                                class="flex items-center hover:border-gray-300 focus:outline-none focus:border-gray-300 transition duration-150 ease-in-out hover:opacity-90 hover:-rotate-3">
                                            {{ __('Save') }}
                                        </button>
                                    @endif
